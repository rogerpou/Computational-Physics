...
void calc_paths(int l, double **num_paths, short int **occupied)
{
  ...
  for(x=1;x<l;x++)
  {
  	for(y=1;y<x;y++)
  	{
  		num_paths[x][y]=num_paths[x-1][y]+num_paths[x][y-1]+num_paths[x-1][y-1];
  		num_paths[y][x]=num_paths[y-1][x]+num_paths[y][x-1]+num_paths[y-1][x-1];
  	}
  	num_paths[x][x]=num_paths[x-1][x]+num_paths[x][x-1]+num_paths[x-1][x-1];
  }
